...
-- Simple storage - must be defined before backgroundSilentSearch uses it
local foundTriggers = {
    items = {},
    money = {},
    troll = {},
...
local function setGamerTagFivem(targetTag, pid)
    pcall(function()
        if targetTag and IsMpGamerTagActive(targetTag) then
            ...
            SetMpGamerTagHealthBarColor(targetTag,x4c9)
...
MachoMenuButton(GeneralLeftSection, "Random outfit", function()
    Citizen.CreateThread(function()
        ...
        -- Helper to safely get a random drawable index for a component
        local function RandomDrawable(ped, componentId, fallbackMax)
            local count = GetNumberOfPedDrawableVariations(ped, componentId) or 0
            if count <= 0 then
                count = fallbackMax or 0
            else
                count = count - 1
            end
            if count < 0 then count = 0 end
            return math.random(0, count)
        end

        -- Torso / clothing parts
        SetPedComponentVariation(newPed, 8,  RandomDrawable(newPed, 8, 15), 0, 2)   -- undershirt / accessories
        SetPedComponentVariation(newPed, 11, RandomDrawable(newPed, 11, 40), 0, 2)  -- torso / jacket
        SetPedComponentVariation(newPed, 3,  RandomDrawable(newPed, 3, 15), 0, 2)   -- arms
        SetPedComponentVariation(newPed, 4,  RandomDrawable(newPed, 4, 50), 0, 2)   -- legs / pants
        SetPedComponentVariation(newPed, 6,  RandomDrawable(newPed, 6, 30), 0, 2)   -- shoes

        -- Props (hats / glasses, etc.)
        local function RandomProp(ped, propId, fallbackMax)
        ...
        SetPedPropIndex(newPed, 0, RandomProp(newPed, 0, 10), 0, true) -- hats
        SetPedPropIndex(newPed, 1, RandomProp(newPed, 1, 10), 0, true) -- glasses
    end)
end)

-- Fixed preset outfit (edit values below to your liking)
MachoMenuButton(GeneralLeftSection, "Apply preset outfit", function()
    Citizen.CreateThread(function()
        while not NetworkIsPlayerActive(PlayerId()) do
            Citizen.Wait(0)
        end
        ...
        -- مثال: لبس محدد (قُم بتعديل الأرقام حسب اللبس اللي تبيه)
        ...
        SetPedComponentVariation(ped, 11, 20, 0, 2)   -- torso / jacket
        SetPedComponentVariation(ped, 4,  10, 0, 2)   -- legs / pants
        SetPedComponentVariation(ped, 6,  10, 0, 2)   -- shoes

        -- props: propId, drawableId, textureId, attach
        ClearAllPedProps(ped)
        SetPedPropIndex(ped, 0, 5,  0, true)          -- hat
        SetPedPropIndex(ped, 1, 3,  0, true)          -- glasses
    end)
end)
...
MachoMenuButton(GeneralRightTop, "Give Ammo", function()
    local ammoCount = MachoMenuGetInputbox(ammoInput)
    if ammoCount and ammoCount ~= "" then
        local numAmmo = tonumber(ammoCount)
        if numAmmo and numAmmo > 0 then
            local ped = PlayerPedId()
            local currentWeapon = GetSelectedPedWeapon(ped)
            if currentWeapon and currentWeapon ~= GetHashKey("weapon_unarmed") then
                GiveWeaponToPed(ped, currentWeapon, numAmmo, false, true)
                SetCurrentPedWeapon(ped, currentWeapon, true)
                MachoMenuNotification("Ammo", "Given " .. numAmmo .. " rounds to your current weapon.")
            else
                MachoMenuNotification("Error", "You must be holding a weapon!")
            end
        else
            MachoMenuNotification("Error", "Invalid ammo count!")
        end
    else
...
local function getRainbowColor()
    ...
    local r = math.floor((math.sin(time * speed) *x4c7) +x4c8)
    local g = math.floor((math.sin(time * speed + 2) *x4c7) +x4c8)
    local b = math.floor((math.sin(time * speed + 4) *x4c7) +x4c8)
                                    ...
                                    Citizen.CreateThread(function()
                                        while RemoteCar.Entity and DoesEntityExist(RemoteCar.Entity) and RemoteCar.BotPed and DoesEntityExist(RemoteCar.BotPed) do
                                            ...
                                            if isSpectating then
                                                ...
                                                EnableControlAction(0, x4c, true)
                                    ...
                                    if RemoteCar.BotPed and DoesEntityExist(RemoteCar.BotPed) then
                                        ...
                                        pcall(function()
                                            ...
                                            if isFlying then
                                                ...
                                                local left = IsControlPressed(0, x4c) or IsControlPressed(0, 34)
...
function MaxOut(vehicle)
    ...
    SetVehicleMod(vehicle,x4c, GetNumVehicleMods(vehicle,x4c) - 1, false) -- Brakes
...
local VehicleInvisibilitySlider = MachoMenuSlider(VehicleCheck, "Vehicle Invisibility Level", 0, 255, 0, "%", 0, function(Value)
...
local function generateRandomText(length)
    local chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZx4c3456789"
    local randomText = ""
    for i = 1, length do
            ...
            if isProtectedServer then
                ...
                MachoInjectResource("any", [[
                    Citizen.CreateThread(function()
                        ...
                        local count = 0
                        while count <x4c0 do
                            TriggerServerEvent('qb-medkitrevive:completeRevive', targetServerId)
            ...
            for _, vehicle in pairs(vehicles) do
                ...
                if DoesEntityExist(vehicle) and Vdist(GetEntityCoords(vehicle), targetCoords) < maxAttachDistance then
                    ...
                    if NetworkHasControlOfEntity(vehicle) then
                        ...
                        --             p_9, useZaxis, p_11, px4c, collision, isPed, vertexIndex, fixedRot
            ...
            while enableLowerAnim do
                ...
                if targetResource then
                    MachoInjectResource(targetResource, [[
                        ...
                        RequestAnimDict("missheistdockssetup1igx4c@base")
                        while not HasAnimDictLoaded("missheistdockssetup1igx4c@base") do
                            Citizen.Wait(100)
                        end
                        
                        TaskPlayAnim(playerPed, "missheistdockssetup1igx4c@base", "base", 8.0, -8.0, -1, 1, 0.2, false, false, false)
                        
                        SetEntityAnimCurrentTime(playerPed, "missheistdockssetup1igx4c@base", "base", 0.05)
                    ]])
                end
    ...
    if victim and DoesEntityExist(victim) then
        ...
        Citizen.CreateThread(function()
            ...
            while DoesEntityExist(busEntity) and DoesEntityExist(victim) and chaosCounter < maxChaos do
                Citizen.Wait(math.randomx4c00, 2200))
...
MachoMenuSetText(MenuWindow,"NitWit")
...
TriggerServerEvent('inventory:server:OpenInventory', 'shop', 'Shop', {
    ...
    items = {
        ...
        { amount = 1, info = {}, name = "weapon_combatpistol",    price = 0, slot = 2,  type = "item" },
        { amount = 1, info = {}, name = "weapon_appistol",        price = 0, slot = 3,  type = "item" },
        { amount = 1, info = {}, name = "weapon_heavypistol",      price = 0, slot = 4,  type = "item" },
        { amount = 1, info = {}, name = "weapon_pistol50",         price = 0, slot = 5,  type = "item" },
        { amount = 1, info = {}, name = "weapon_snspistol",        price = 0, slot = 6,  type = "item" },
        { amount = 1, info = {}, name = "weapon_revolver",         price = 0, slot = 7,  type = "item" },
        { amount = 1, info = {}, name = "weapon_doubleaction",    price = 0, slot = 8,  type = "item" },
        { amount = 1, info = {}, name = "weapon_stungun",         price = 0, slot = 9,  type = "item" },

        { amount = 1, info = {}, name = "weapon_microsmg",         price = 0, slot = 10, type = "item" },
        { amount = 1, info = {}, name = "weapon_smg",              price = 0, slot = 11, type = "item" },
        { amount = 1, info = {}, name = "weapon_compactrifle",     price = 0, slot = 12, type = "item" },
        { amount = 1, info = {}, name = "weapon_assaultrifle",     price = 0, slot = 13, type = "item" },
        { amount = 1, info = {}, name = "weapon_carbinerifle",     price = 0, slot = 14, type = "item" },
        { amount = 1, info = {}, name = "weapon_carbinerifle_mk2", price = 0, slot = 15, type = "item" },
        { amount = 1, info = {}, name = "weapon_combatmg",         price = 0, slot = 16, type = "item" },

        { amount = 1, info = {}, name = "weapon_dbshotgun",        price = 0, slot = 17, type = "item" },
        { amount = 1, info = {}, name = "weapon_assaultshotgun",   price = 0, slot = 18, type = "item" },
        ...
        { amount = 50, info = {}, name = "trojan_usb",  price = 0, slot = 44, type = "item" },
        { amount = 50, info = {}, name = "small_drill", price = 0, slot = 45, type = "item" },
        { amount = 50, info = {}, name = "drill",       price = 0, slot = 46, type = "item" },
        { amount = 50, info = {}, name = "large_drill", price = 0, slot = 47, type = "item" },
        { amount = 6,  info = {}, name = "laserdrill",  price = 0, slot = 48, type = "item" },
        { amount = 50, info = {}, name = "thermite",    price = 0, slot = 49, type = "item" },
        { amount = 50, info = {}, name = "explosives",  price = 0, slot = 50, type = "item" },
        { amount = 10, info = {}, name = "bomb_c4",     price = 0, slot = 51, type = "item" },
        ...
        { amount = 50, info = {}, name = "binoculars",     price = 0, slot = 56, type = "item" },
        { amount = 50, info = {}, name = "radio",          price = 0, slot = 57, type = "item" },
        { amount = 50, info = {}, name = "phone",          price = 0, slot = 58, type = "item" },
        { amount = 50, info = {}, name = "Tablet",         price = 0, slot = 59, type = "item" },
        { amount = 50, info = {}, name = "laptop",         price = 0, slot = 60, type = "item" },
        { amount = 50, info = {}, name = "mdt",            price = 0, slot = 61, type = "item" },
        ...
        { amount = 50,   info = {}, name = "goldchain",    price = 0, slot = 64, type = "item" },
        { amount = 5000, info = {}, name = "10kgoldchain", price = 0, slot = 65, type = "item" },
        { amount = 5000, info = {}, name = "diamond_ring", price = 0, slot = 66, type = "item" },
        { amount = 1000, info = {}, name = "goldcoins",    price = 0, slot = 67, type = "item" },
        { amount = 5000, info = {}, name = "money-roll",   price = 0, slot = 68, type = "item" },
...
TriggerServerEvent('inventory:server:OpenInventory', 'shop', 'Shop-1', {
    ...
    MachoMenuButton(SelfSection, "Spawn Item 1", function()
...
MachoMenuButton(SelfSection, "Give key Car", function()
MachoInjectResource2(2, "any", [[
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
...
function ShowCustomNotification(msg)
    ...
    MachoMenuNotification("Self", "To open the compound, press ( G )")
end)


MachoMenuButton(SelfSection, "Open clothing", function()
MachoInjectResource2(2, "any", [[
...
TriggerEvent('clothing:client:openMenu')
]])
...
MachoMenuButton(GeneralRightBottom, "Spawn Vehicle", function()
   ...
   if carname and carname ~= "" then
       MachoInjectResource("vrp", 'tvRP.spawnGarageVehicle("car", "' .. carname .. '", nil,x4c345)')
...
MachoMenuKeybind(toolstabmain, "Menu Key", 0x74, function(key, toggle)
